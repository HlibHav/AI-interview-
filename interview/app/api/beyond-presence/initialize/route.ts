...
export async function POST(request: NextRequest) {
  try {
    const { sessionId, participantEmail, researchGoal, script } = await request.json();
    ...
    // Build system prompt based on research goal and script
    let systemPrompt = `You are a professional user research interviewer conducting a qualitative research session. 
...
Start by introducing yourself and explaining the purpose of the research session.`;
    ...
    // If we have a script, include it in the prompt
    if (script?.questions && script.questions.length > 0) {
      systemPrompt += `\n\nInterview Script:
${script.introduction || ""}

Questions to cover:
${script.questions.map((q: any, idx: number) => `${idx + 1}. ${q.question}${q.intent ? ` (Intent: ${q.intent})` : ""}`).join("\n")}

${script.closing ? `\nClosing: ${script.closing}` : ""}

${script.reminders?.length ? `\nReminders: ${script.reminders.join(", ")}` : ""}

Follow these questions but feel free to ask follow-ups based on the participant's responses. Adapt the conversation naturally.`;
    }
    ...
    const response = await fetch(`${process.env.BEY_API_URL || 'https://api.bey.dev'}/v1/agents`, {
      ...
      body: JSON.stringify({
        ...
        language: "en",
        greeting: script?.introduction || "Hello! I'm your AI research interviewer. I'm here to learn about your experiences and preferences to help improve our products and services. Are you ready to begin?",
